...
def build_forced_prompts(prompts, target_item, items):
    ...
    for p in prompts:
        ...
        neg = p + f"\nAnswer: The best choice is{other}"
...
def calculate_directions(model_base, item_name, domain, items_dir, personas_dir, prompt_dir, save_dir):
    ...
    _, layer, direction = select_direction(
		...
		positive_prompts,
		candidate_direction,
		artifact_dir=os.path.join(save_direction(item_name, save_dir), "select_direction"),
